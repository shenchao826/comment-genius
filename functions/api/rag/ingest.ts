const SEED_DATA = [
  { title: '小学低段数学正面评语-学习兴趣与计算能力', content_type: 'teacher_comment_math', content: [
  ...
  { title: '小学高段语文改进建议-作文与阅读', content_type: 'teacher_comment_chinese', content: [
  ...
  { title: '初中英语鼓励寄语-发音与参与度', content_type: 'teacher_comment_english', content: [
  ...
  { title: '班主任综合评语-德智体美劳全面发展', content_type: 'teacher_comment_homeroom', content: [
  ...
  { title: '教育评价原则-五条核心原则', content_type: 'evaluation_principle', content: [
  ...
  { title: '语言风格模板-开头过渡句结尾鼓励句', content_type: 'language_template', content: [
...
async function checkExists(baseUrl: string, apiKey: string, title: string): Promise<boolean> {
  let urlStr = baseUrl + 'kb_documents';
  urlStr += '?select=id';
  urlStr += '&title=eq.' + encodeURIComponent(title);
  urlStr += '&limit=1';
  const res = await fetch(urlStr, { headers: { apikey: apiKey, Authorization: 'Bearer ' + apiKey } });
  if (!res.ok) return false;
  const data = (await res.json()) as any[];
  return Array.isArray(data) && data.length > 0;
}

async function insertToSupabase(baseUrl: string, apiKey: string, doc: typeof SEED_DATA[0]): Promise<{ ok: boolean; detail: string; skipped: boolean }> {
  const exists = await checkExists(baseUrl, apiKey, doc.title);
  if (exists) return { ok: true, detail: '已存在，跳过', skipped: true };
  const urlStr = baseUrl + 'kb_documents';
  const res = await fetch(urlStr, {
    method: 'POST',
    headers: { apikey: apiKey, Authorization: 'Bearer ' + apiKey, 'Content-Type': 'application/json', Prefer: 'return=representation' },
    body: JSON.stringify({ title: doc.title, content: doc.content, content_type: doc.content_type, product_line: 'teachers', is_published: true }),
  });
  let detail = '';
  if (!res.ok) { try { detail = await res.text(); } catch (e) { /* ignore */ } }
  return { ok: res.ok, detail: res.status + ': ' + detail, skipped: false };
}
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    let skipped = 0;
    const errors: string[] = [];
    for (let di = 0; di < SEED_DATA.length; di++) {
      const result = await insertToSupabase(sbUrl, sbKey, SEED_DATA[di]);
      if (result.skipped) { skipped++; } else if (result.ok) { inserted++; } else { errors.push(SEED_DATA[di].title + ': ' + result.detail); }
    }

    const docsInfo = SEED_DATA.map((doc) => ({ title: doc.title, type: doc.content_type }));
    ...
    return Response.json({ error: '种子数据录入失败', detail: String(e) }, { status: 500 });
  }
};
...
export const onRequestGet:PagesFunction<Env>=async(context)=>{
  try {
    ...
    let sbUrl: string | undefined, sbKey: string | undefined;
    try { sbUrl = env.SUPABASE_URL; sbKey = env.SUPABASE_KEY; } catch (e) { /* env vars may not be defined */ }
    if (!sbUrl || !sbKey) return Response.json({ error: 'Supabase未配置' }, { status: 503 });

    let urlStr = sbUrl + 'kb_documents';
    urlStr += '?select=id,title,content_type,is_published,created_at';
    urlStr += '&limit=20';
    urlStr += '&order=created_at.desc';

    const res = await fetch(urlStr, { headers: { apikey: sbKey, Authorization: 'Bearer ' + sbKey } });
    if (!res.ok) return Response.json({ error: '查询失败' }, { status: 500 });
    ...
    return Response.json({ stats: { total: (docs || []).length }, documents: resultDocs });
  } catch (e) {
    return Response.json({ error: '查询失败', detail: String(e) }, { status: 500 });
  }
};
